...
func (plugin *cniNetworkPlugin) SetUpPod(namespace string, name string, id kubetypes.DockerID, containerRuntime string) error {
...
func (plugin *cniNetworkPlugin) TearDownPod(namespace string, name string, id kubetypes.DockerID, containerRuntime string) error {
	runtime, ok := plugin.host.GetRuntime().(*dockertools.DockerManager)
	if !ok {
		return fmt.Errorf("CNI execution called on non-docker runtime")
	}
...
func (plugin *cniNetworkPlugin) Status(namespace string, name string, id kubetypes.DockerID, containerRuntime string) (*network.PodNetworkStatus, error) {
	runtime, ok := plugin.host.GetRuntime().(*dockertools.DockerManager)
	if !ok {
