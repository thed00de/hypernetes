...
type NetworkPlugin interface {
	...
	// pod are launched.
	SetUpPod(namespace string, name string, podInfraContainerID kubetypes.DockerID, containerRuntime string) error

	// TearDownPod is the method called before a pod's infra container will be deleted
	TearDownPod(namespace string, name string, podInfraContainerID kubetypes.DockerID, containerRuntime string) error

	// Status is the method called to obtain the ipv4 or ipv6 addresses of the container
	Status(namespace string, name string, podInfraContainerID kubetypes.DockerID, containerRuntime string) (*PodNetworkStatus, error)
}
...
func (plugin *noopNetworkPlugin) SetUpPod(namespace string, name string, id kubetypes.DockerID, containerRuntime string) error {
	return nil
}

func (plugin *noopNetworkPlugin) TearDownPod(namespace string, name string, id kubetypes.DockerID, containerRuntime string) error {
	return nil
}

func (plugin *noopNetworkPlugin) Status(namespace string, name string, id kubetypes.DockerID, containerRuntime string) (*PodNetworkStatus, error) {
	return nil, nil
}
